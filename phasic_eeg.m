...
% band = 'alpha';
band = 'theta';
% FILTER_TYPE = 'alpha_stimenc';
FILTER_TYPE = 'theta_stimenc';
% FILTER_TYPE = 'ERP';
EVENT_TAG = {'encm'};
...
%         EPOCH_TYPE = 'alpha_stimenc';
        EPOCH_TYPE = 'theta_stimenc';

        [ALLEEG, EEG, CURRENTSET, ALLCOM] = eeglab;
        file_to_load = 'merged_raw_epochs.set';
        EEG = pop_loadset(file_to_load);

        if strcmp(EPOCH_TYPE, 'alpha_stimenc')
        ...
        if strcmp(EPOCH_TYPE, 'theta_stimenc')
            MAN_REJ_FNAME = strcat(SUBJ_TO_INSPECT, '_', EPOCH_TYPE, '_rejected_epochs.csv');
...
%             EEG = pop_rmbase(EEG, ERP_BASELINE_TIMEBIN);
    ...
    if strcmp(step, 'filter')
        ...
        if strcmp(FILTER_TYPE, 'alpha_stimenc') | strcmp(FILTER_TYPE, 'theta_stimenc')
            %% FILTER STEP
            for i = 1:n_runs
                %% Setup
                r = num2str(runs(i));
